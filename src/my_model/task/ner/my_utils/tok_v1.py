...
class TOK:
	def __init__():
        self.lang = 'en'
        self.nonbreaking_prefix = open('nbp').read().decode('utf8').split()
        self.units = open('units').read().decode('utf8').split()
    def is_number(w):
        # REF:https://en.wikipedia.org/wiki/Decimal_separator
        thousand_sep = ','
        decimal_sep = '.'
        if self.lang in ['pt','es','ru','mo','uk']:
    ...
    def proc_unit(w):
        for u in self.units:
            if w[-len(u):] == u and is_number(w[:-len(u)],self.lang):
    ...
    def tok(s):
        ...
        final_tok_ws = []
        for w in pre_tok_ws:
            if is_url_or_filename(w) or all(c=='.' for c in w) or w.lower() in self.nonbreaking_prefix:
                final_tok_ws.append(w)
            elif w[-1] == '.':
                w = proc_unit(w[:-1],self.units,self.lang)
                final_tok_ws += [w,'.']
            else:
                w = proc_unit(w,self.units,self.lang)
                final_tok_ws += [w]
        #o = ' '.join(final_tok_ws).encode('utf8')
        return final_tok_ws
    def tokend_line(line):        
       return tok(line,nonbreaking_prefix,units,lang)
