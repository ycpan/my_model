...
class NER_MODEL(ABC_MODEL):
    ...
    def train(self):
        ...
        train_example_file = self.save_example_tf_record(self.args.train_data,'train')
        eval_example_file = self.save_example_tf_record(self.args.eval_data,'eval')
        train_inpf = self.get_input_fn(train_example_file,self.args.batch_size)
        eval_inpf = self.get_input_fn(eval_example_file,self.args.batch_size)
    ...
    def model_fn_builder(self,args):
        ...
        #
            ...
            output = embeddings
            if args.use_lstm:
                t = tf.transpose(output, perm=[1, 0, 2])
        ...
        def model_fn(features, labels, mode, params):
            ...
            (total_loss, logits,predicts) = create_model(embeddings,label_ids,mask, training, self.num_labels,use_one_hot_embeddings=False)
            ...
            if mode == tf.estimator.ModeKeys.TRAIN:
                ...
                metrics = {
                    ...
                    'precision': precision(label_ids, pred_ids, self.num_labels, self.indices, weights),
