...
if os.name == 'nt':
    ...
    bert_path = './checkpoint1'
    root_path = './class_input'
...
def create_model(bert_config, is_training, input_ids, mode, input_mask=None, segment_ids=None, labels=None, num_labels=None):
    """
    ...
    trigger_output_layer = model.get_pooled_output()
    hidden_size = trigger_output_layer.shape[-1].value
    ...
    if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
        with tf.variable_scope("loss"):
            if is_training:
                # I.e., 0.1 dropout
                trigger_output_layer = tf.nn.dropout(trigger_output_layer, keep_prob=0.9)

            logits = tf.matmul(trigger_output_layer, output_weights, transpose_b=True)
        ...
        return (None, None, logits, probabilities)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate, num_train_steps,
    ...
    def model_fn(features, labels, mode, params):
        ...
        (total_loss, per_example_loss, logits, probabilities) = create_model(
            bert_config, is_training, input_ids, mode, input_mask, segment_ids, label_ids,
            num_labels)
