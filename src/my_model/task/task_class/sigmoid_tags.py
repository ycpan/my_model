from my_model.task.task_class.abc_multi_tags import ABCMulTags
#import functools
import json
import logging
import sys
...
class CLASS_MODEL(ABCMulTags):
    ...
    def train(self):
        ...
        train_inpf = self.get_input_fn(train_example_file,self.args.batch_size,is_training=True,drop_remainder=True)
        eval_inpf = self.get_input_fn(eval_example_file,self.args.batch_size,is_training=False,drop_remainder=False)
...
#        if not args.glove :
#            args.glove = os.path.join(args.eval_data_dir,'glove.npz')
#        return NER_CRF_CLASS_MODEL.model_fn_builder(args)
#    
