...
# @Author  : 林利芳
# @File    : modules.py
# copy from transformer_crf:https://github.com/phychaos/transformer_crf
...
def positional_encoding(inputs, num_units, zero_pad=True, scale=True, scope="positional_encoding", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        position_enc = np.array([
            [pos / np.power(10000, 2. * i / num_units) for i in range(num_units)] for pos in range(T)], dtype=np.float32)
